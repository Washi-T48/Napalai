...
const getForgottenList = async () => {
    const rows = await pool.query('SELECT *, camera.name AS cameraName, zone.name AS zoneName, forgotten.id as forgottenID, forgotten.created as createdTime FROM forgotten LEFT JOIN event ON forgotten.event_id = event.id LEFT JOIN camera ON event.camera_id = camera.id LEFT JOIN zone ON camera.zone_id = zone.id ORDER BY event.created DESC');
    return rows;
};

const getForgottenById = async (id) => {
    const rows = await pool.query('SELECT *, camera.name AS cameraName, zone.name AS zoneName, forgotten.id as forgottenID, forgotten.created as createdTime FROM forgotten LEFT JOIN event ON forgotten.event_id = event.id LEFT JOIN camera ON event.camera_id = camera.id LEFT JOIN zone ON camera.zone_id = zone.id WHERE forgotten.id = $1 ORDER BY event.created DESC', [id]);
    return rows;
};

const getViolenceList = async () => {
    const rows = await pool.query('SELECT *, camera.name AS cameraName, zone.name AS zoneName, violence.id as violenceID, violence.created as createdTime FROM violence LEFT JOIN event ON violence.event_id = event.id LEFT JOIN camera ON event.camera_id = camera.id LEFT JOIN zone ON camera.zone_id = zone.id ORDER BY event.created DESC');
    return rows;
};

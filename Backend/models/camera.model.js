...
const changeCameraZone = async (id, camera) => {
    const { zone_id } = camera;
    const rows = await pool.query('UPDATE camera SET zone_id = $1 WHERE id = $2 RETURNING id, zone_id', [zone_id, id]);
    return rows;
};
